...
def Experiment(mg, Model,
    ...
    ####################
    unsIndex = [i for i in range(len(mg.varmapper.keys())) if 'u_' in mg.varmapper[i]]
        ...
        for i,k in mg.varmapper.items():
            ...
            for g in mg.genelist:
                if g in icsmap.keys():
                    ...
                    # we assume 20% of initial condition is unspliced
                    ss[revvarmapper['u_'+g]] = icsmap[g]*0.2
                    ss[revvarmapper['x_'+g]] = icsmap[g]*0.8
                else:
                    ss[revvarmapper['u_'+g]] = 0.01
    ...
    if len(mg.proteinlist) == 0:
        ...
        speciesoi.extend([revvarmapper['u_' + g] for g in mg.genelist])
    ...
    argdict['writeUnspliced'] = writeUnspliced
    ...
    # here we remove x, but keep u_ to distinguish 
...
def startRun(settings):
    ...
    resultDF = Experiment(mg, model.Model,
                          ...
                          writeUnspliced=settings['writeUnspliced'],
...
def simulateAndSample(argdict):
    ...
    while retry:
        seed += 1000
        y0_exp = simulator.getInitialCondition(ss, ModelSpec, unsIndex, rnaIndex, proteinIndex,
