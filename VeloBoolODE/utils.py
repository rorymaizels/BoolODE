...
def loadParameterValues():
    ...
    # Max level checks
    u_max = kineticParameterDefaults['mRNATranscription']/kineticParameterDefaults['mRNASplicing']
    x_max = u_max*(kineticParameterDefaults['mRNASplicing']/kineticParameterDefaults['mRNADegradation'])
    y_max = x_max*(kineticParameterDefaults['proteinTranslation']/kineticParameterDefaults['proteinDegradation'])

    if u_max != 1.0:
        print('Warning: max(uns_mRNA) != 1.0')
    if x_max != 2.0:
...
def sampleTimeSeries(num_timepoints, expnum,\
    ...
    unsIndex = [i for i in range(len(varmapper.keys())) if 'u_' in varmapper[i]]
    ...
    if writeProtein:
        if writeUnspliced:
            ...
            print("Write Protein but non Unspliced not implemented. I'm sorry. Setting writeUnspliced=True")
            # Write protein and mRNA to file
            for ri in varmapper.keys():
                sampleDict[varmapper[ri]] = {h:P[ri][ti] for h,ti in zip(experimentTimePoints,timeIndex)}
    ...
    if writeProtein:
        ...
        if len(proteinlist) == 0:
            for ri in rnaIndex:
                sampleDict[varmapper[ri]] = {header[cellid]: P[ri][timepoint]}
            for ui in unsIndex:
                sampleDict[varmapper[ui]] = {header[cellid]: P[ui][timepoint]}
        else:
            speciesoi = [revvarmapper['p_' + p] for p in proteinlist]
            speciesoi.extend([revvarmapper['x_' + g] for g in genelist])
            speciesoi.extend([revvarmapper['u_' + g] for g in genelist])
